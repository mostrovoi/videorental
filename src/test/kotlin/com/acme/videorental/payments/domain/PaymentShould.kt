...
class PaymentShould {
    ...
    fun return_no_charge_when_return_is_on_time() {
        ...
        val totalToPay = Payment.calculatePriceForOverdueReturn(10, rentalTime, returnTime, paidRentedDays)
        Assertions.assertThat(totalToPay).isEqualTo(Payment.NO_SURCHARGE)
    }
    ...
    fun throw_exception_on_calculating_surcharge_when_rental_time_is_after_return_time() {
        ...
        val paidRentedDays = 1
        Payment.calculatePriceForOverdueReturn(10, rentalTime, returnTime, paidRentedDays)
    }
    ...
    fun throw_exception_on_calculating_surcharge_when_price_is_less_or_equal_than_zero() {
        ...
        val returnTime = Instant.now()
        val paidRentedDays = 1
        Payment.calculatePriceForOverdueReturn(0, rentalTime, returnTime, paidRentedDays)
    }

    @Test
    fun calculate_rental_price_for_regular_movie_for_multiple_days() {
        val totalPrice = Payment.calculatePrice(FilmTypeEnum.REGULAR, 5)
        Assertions.assertThat(totalPrice).isEqualTo(90)
    }

    @Test
    fun calculate_rental_price_for_old_movie_for_multiple_days() {
        val totalPrice = Payment.calculatePrice(FilmTypeEnum.OLD, 7)
        Assertions.assertThat(totalPrice).isEqualTo(90)
    }
}